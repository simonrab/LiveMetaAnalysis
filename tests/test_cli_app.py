...
def test_run_demo_reports_and_saves(store, capsys):
    code = main(argv=["run", "--demo"], fetch_study=_fetch, store=store)
...
def test_run_demo_json_is_clean(store, capsys):
    code = main(argv=["run", "--demo", "--json"], fetch_study=_fetch, store=store)
...
def test_run_demo_no_save_persists_nothing(store):
    main(argv=["run", "--demo", "--no-save"], fetch_study=_fetch, store=store)
