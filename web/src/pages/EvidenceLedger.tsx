...
function Prisma({ review }: { review: ReviewResult }) {
  ...
  return (
    <section className="overflow-x-auto rounded-md border border-hairline-light bg-card-light p-4">
      <div className="flex min-w-max items-center gap-3">
        <span className="w-16 text-right text-[11px] font-semibold uppercase tracking-wider text-ink-muted-light">
          PRISMA
        </span>
        ...
        {steps.map(([n, label, hl], i) => (
          <div key={label} className="flex items-center gap-3">
            <div className="flex flex-col items-center">
              <span
                className={`font-mono text-[13px] ${hl ? "font-bold text-secondary" : "text-ink-light"}`}
              >
                {n}
              </span>
              <span
                className={`mt-1 text-[10px] font-semibold uppercase tracking-wider ${
                  hl ? "text-secondary" : "text-ink-muted-light"
                }`}
              >
                {label}
              </span>
    ...
    <div className="mx-auto max-w-6xl space-y-6 px-8 py-10">
      <div className="flex items-end justify-between gap-4">
        ...
        <Link
          ...
          className="rounded-sm border border-hairline-light px-4 py-2 text-[13px] font-medium text-ink-light hover:bg-surface-container-high"
        >
          View report →
        </Link>
      </div>
      ...
      <section className="overflow-x-auto rounded-md border border-hairline-light bg-card-light">
        <div className="grid min-w-[720px] grid-cols-12 gap-3 border-b border-hairline-light bg-surface-container-low p-3 text-[11px] font-semibold uppercase tracking-wider text-ink-muted-light">
        ...
        {review.extractions.map((e) => {
          ...
          return (
            <div
              key={e.study_id}
              className="grid min-w-[720px] grid-cols-12 items-center gap-3 border-b border-hairline-light p-3 last:border-0 hover:bg-surface-container-low"
            >
              <div className="col-span-4 min-w-0">
                <Link
                  to={`/reviews/${id}/evidence/${e.study_id}`}
                  className="block truncate text-[14px] text-ink-light hover:text-secondary"
                >
                  {e.label}
                </Link>
              ...
              <div className="col-span-3 flex items-center gap-2">
                {e.hr != null ? (
                  <>
                    <span className="rounded-sm border border-hairline-light bg-surface-container px-1.5 py-0.5 font-mono text-[13px] text-ink-light">
